document.addEventListener('DOMContentLoaded', () => {
    ...
    const trainerInfo = document.querySelector("#trainer-info span");
            ...
            if (sportSection) {
                ...
                switch (sportSection) {
                    ...
                    case "boxing":
                        ...
                        break;
                    case "tennis":
                        body.style.backgroundImage = "url('../img/теннис.jpg')";
                        break;
...
// Функция для удаления элемента инвентаря
        ...
        document.getElementById("statistics").addEventListener("click", function () {
            ...
            const buildTelegramLink = tg => tg ? `tg://user?id=${tg}` : null;

            const processUserAll = (count) => {
                let infoBox3 = document.createElement('div');
                infoBox3.classList.add('info-box3');
                const countPeople = count > 0 ? count : "Нету зарегистрированных учеников";
                infoBox3.innerHTML = `
            ...
            <div id="userSectionPanel" style="display: none; margin-top: 10px;">
                <h4>Выбор секции</h4>
                <select id="sportSectionSelect" class="form-select mb-3"></select>
                <div id="userList"></div>
            </div>`;
                return infoBox3;
            };

            async function loadSportSectionsForCouch() {
                const select = document.getElementById("sportSectionSelect");
                const userList = document.getElementById("userList");

                try {
                    const response = await fetch("/api/couch/sport-section/");
                    ...
                    select.onchange = () => {
                        ...
                        if (sectionId) {
                            loadUsersBySportSection(sectionId);
                        } else {
                            userList.innerHTML = '';
                        }
                    };
                } catch (e) {
                    console.error("Ошибка при загрузке секций", e);
                }
            }

            async function loadUsersBySportSection(sectionId) {
                const userListDiv = document.getElementById("userList");
                userListDiv.style.display = "block";
                userListDiv.innerHTML = "<h5>Загрузка учеников...</h5>";

                try {
                    const response = await fetch(`/api/couch/getAllUsersBySection/${sectionId}`);
                    const users = await response.json();
                    userListDiv.innerHTML = "<h5>Ученики выбранной секции:</h5>";

                    const today = new Date();
                    if (!users.length) {
                        userListDiv.innerHTML += "<p>Нет учеников в данной секции.</p>";
                        return;
                    }

                    users.forEach(user => {
                        const fullName = `${user.name} ${user.surname}`;
                        const key = `${user.name?.trim().toLowerCase()} ${user.surname?.trim().toLowerCase()}`.trim();
                        const tg = tgMap.get(key);

                        let color = 'gray';
                        let statusText = 'Тариф не приобретён';

                        if (user.ticket?.startDate && user.ticket?.days) {
                            const startDate = new Date(user.ticket.startDate);
                            const endDate = new Date(startDate);
                            endDate.setDate(endDate.getDate() + user.ticket.days);
                            if (endDate >= today) {
                                color = 'green';
                                statusText = `Активен до ${endDate.toLocaleDateString()}`;
                            } else {
                                color = 'red';
                                statusText = `Просрочен (${endDate.toLocaleDateString()})`;
                            }
                        }

                        userListDiv.innerHTML += `
                    <p style="color: ${color}; font-weight: bold;">
                        ${tg ? `<a href="${buildTelegramLink(tg)}" target="_blank">${fullName}</a>` : fullName}
                         — ${statusText}
                    </p>`;
                    });

                } catch (e) {
                    userListDiv.innerHTML = `<p class="text-danger">Ошибка загрузки: ${e.message}</p>`;
                }
            }

            async function fetchData() {
                // Загружаем TG map
                const allUsersForTg = await fetch("/api/user/userForTg/").then(r => r.json());
                const normalizeName = (name, surname = '') =>
                    `${(name || '').trim().toLowerCase()} ${(surname || '').trim().toLowerCase()}`.trim();
                allUsersForTg.forEach(u => {
                    tgMap.set(normalizeName(u.name, u.surname), u.tg);
                });

                // Загружаем количество учеников
                const usersData = await fetch('/api/couch/allUserForCouch/').then(r => r.json());
                const count = usersData.filter(Boolean).length;
                const userBlock = processUserAll(count);
                document.getElementById("users-panel").innerHTML = '';
                document.getElementById("users-panel").appendChild(userBlock);

                // Кнопка показать/скрыть
                const showBtn = document.getElementById("showAllUsers");
                const userPanel = document.getElementById("userSectionPanel");
                showBtn.addEventListener("click", () => {
                    if (userPanel.style.display === "none" || !userPanel.style.display) {
                        userPanel.style.display = "block";
                        showBtn.textContent = "Скрыть список учеников";
                    } else {
                        userPanel.style.display = "none";
                        showBtn.textContent = "Показать всех учеников";
                    }
                });

                loadSportSectionsForCouch();

                // Загружаем профиль
                const profileData = await fetch('/api/couch/getCouch/').then(r => r.json());
                profileInfo.innerHTML = '';
                profileInfo.appendChild(processProfileData(profileData));

                // Загружаем расписание
                await renderTrainerStats();
            }
...
// Проверяем, выходит ли за правую границу
    ...
    async function renderTrainerStats() {
        const normalizeType = v => String(v || '').toLowerCase();
        const ruDate = d => d.toLocaleDateString('ru-RU');
        const ruTimeHM = d => d.toLocaleTimeString('ru-RU', {hour: '2-digit', minute: '2-digit'});
        ...
        // 2. Расписание
        const scheduleList = await fetch("/api/schedule/couch/").then(r => r.json());
        const individual = [];
        const generalAll = [];
        ...
        (scheduleList || []).forEach(s => {
            ...
            const type = normalizeType(s.typeWorkout);

            // Индивидуальные
            if (type === 'individual' && d >= today) {
                const namesSet = new Map();

                const addUser = (u) => {
                    if (u?.name) {
                        const fullName = `${u.name}${u.surname ? ' ' + u.surname : ''}`;
                        const key = normalizeName(u.name, u.surname);
                        if (!namesSet.has(key)) {
                            namesSet.set(key, fullName);
                        }
                    }
                };

                if (Array.isArray(s.bookings)) {
                    s.bookings.forEach(b => addUser(b?.bookingUserCouch?.user ?? b?.user));
                }
                if (Array.isArray(s.scheduleDateUser?.userInfoDTO)) {
                    s.scheduleDateUser.userInfoDTO.forEach(addUser);
                }

                individual.push({date: d, names: [...namesSet.values()]});
            }

            // Общие
            if (type === 'general' && d >= today) {
                generalAll.push(d);
            }
        });

        // 3. Вывод — старый дизайн
        let individualHTML = `<strong>Индивидуальные:</strong><br/>`;
        if (individual.length > 0) {
            individual.sort((a, b) => a.date - b.date);
            individual.forEach(item => {
                const dt = `${ruDate(item.date)} ${ruTimeHM(item.date)}`;
                const namesPart = item.names.length
                    ? ' — ' + item.names.map(fullName => {
                    const [n, ...rest] = fullName.split(' ');
                    const key = normalizeName(n, rest.join(' '));
                    const tg = tgMap.get(key);
                    return tg
                        ? `<a href="${buildTelegramLink(tg)}" target="_blank">${fullName}</a>`
                        : fullName;
                }).join(', ')
                    : ' (нет записей)';
                individualHTML += `• ${dt}${namesPart}<br/>`;
            });
        } else {
            individualHTML += `Нет актуальных индивидуальных тренировок<br/>`;
        }

        let generalHTML = `<br/><strong>Общие:</strong><br/>`;
        if (generalAll.length > 0) {
            generalAll.sort((a, b) => a - b);

            const monthsMap = new Map();
            generalAll.forEach(d => {
                const label = d.toLocaleDateString('ru-RU', {month: 'long', year: 'numeric'});
                if (!monthsMap.has(label)) monthsMap.set(label, []);
                monthsMap.get(label).push(d);
            });

            const monthOrder = [...monthsMap.keys()].sort((a, b) => {
                const parse = l => {
                    const [mName, yStr] = l.split(' ');
                    const y = parseInt(yStr, 10);
                    const idx = new Date(Date.parse(`01 ${mName} ${y}`)).getMonth();
                    return {y, idx};
                };
                const A = parse(a), B = parse(b);
                return A.y - B.y || A.idx - B.idx;
            });

            monthOrder.forEach(label => {
                const dates = monthsMap.get(label) || [];
                const weekdays = [...new Set(dates.map(d =>
                    d.toLocaleDateString('ru-RU', {weekday: 'long'})
                ))];
                weekdays.sort((a, b) => weekdaysOrder.indexOf(a) - weekdaysOrder.indexOf(b));
                generalHTML += `${label}: ${weekdays.join(', ')}<br/>`;
            });
        } else {
            generalHTML += `Нет общих тренировок после сегодняшнего дня<br/>`;
        }

        // 4. Вставка в отдельный контейнер
        document.querySelector("#trainer-schedule").innerHTML = individualHTML + generalHTML;
    }

// function updateTrainerInfoBox(scheduleList) {
//     fetch("/api/booking/couchBooking/")
//         .then(res => res.json())
//         .then(bookings => {
//             document.querySelector("#trainer-info span").innerHTML = processBookingData(bookings);
//         });
// }
